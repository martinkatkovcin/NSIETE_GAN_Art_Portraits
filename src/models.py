...
class Discriminator(nn.Module):
    def __init__(self, ndf=64, nc=3, im_size=512):
        ...
        nfc_multi = {4: 16, 8: 16, 16: 8, 32: 4, 64: 2,
                     128: 1, 256: 0.5, 512: 0.25, 1024: 0.125}
        nfc = {}
        for k, v in nfc_multi.items():
            nfc[k] = int(v*ndf)
    ...
    def forward(self, imgs, label, part=None):
        if type(imgs) is not list:
            imgs = [F.interpolate(imgs, size=self.im_size),
...
class TextureDiscriminator(nn.Module):
    def __init__(self, ndf=64, nc=3, im_size=512):
        ...
        nfc_multi = {4: 16, 8: 8, 16: 8, 32: 4, 64: 2,
                     128: 1, 256: 0.5, 512: 0.25, 1024: 0.125}
        nfc = {}
        for k, v in nfc_multi.items():
            nfc[k] = int(v*ndf)
...
def create_models(args, shape, device):
    ...
    result = {
        # MLP generator
        "gen":  Generator(zdim=args.zdim, out_shape=shape),

        # MLP discriminator
        "dis":  Discriminator(in_shape=shape, use_spectral_norm=use_sn)
    }
