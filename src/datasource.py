...
def create_Portraits(args):
    d = ds.ImageFolder(root=DATASET_BASEDIR,
                       transform=tvf.Compose([
                               tvf.Resize(args.image_size),
                               tvf.CenterCrop(args.image_size),
                               tvf.ToTensor(),
                               tvf.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
                           ]))
    shape = (args.channels, args.image_size, args.image_size)
    return d, shape
...
class DataSource:
    def __init__(self, args, device):
        ...
        self.ds, self.shape = DATASETS[args.dataset](args)
